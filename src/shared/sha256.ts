...
const LENGTH_FIELD_BYTE_LENGTH = 8;
...
/** Computes canonical lowercase SHA-256 for exactly the bytes in this view. */
export function sha256Hex(bytes: Uint8Array): string {
  const hash = Uint32Array.from(INITIAL_HASH);
  const schedule = new Uint32Array(64);
  const completeByteLength =
    bytes.byteLength - (bytes.byteLength % BLOCK_BYTE_LENGTH);
  ...
  const paddedByteLength =
    remainingByteLength + 1 + LENGTH_FIELD_BYTE_LENGTH <= BLOCK_BYTE_LENGTH
      ? BLOCK_BYTE_LENGTH
      : BLOCK_BYTE_LENGTH * 2;
  const padded = new Uint8Array(paddedByteLength);
  padded.set(bytes.subarray(completeByteLength));
  padded[remainingByteLength] = 0x80;
